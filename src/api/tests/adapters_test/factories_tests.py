...
class TestFoodFactory:
    def test_if_create_instance_data_is_correct(self , simple_food_data : dict , food_factory : IFoodFactory):
        ...
        assert factory_instance.slug  == simple_food_data.get("slug").lower().replace(" ","-")
    ...
    def test_if_reconstruct_instance_data_is_correct(
        self, simple_food_data: dict, food_factory: IFoodFactory
    ):

        factory_instance = food_factory.reconstruct(**simple_food_data)

        assert isinstance(factory_instance, Food)
        assert factory_instance.id == simple_food_data.get("id")
        assert factory_instance.name == simple_food_data.get("name")
        assert factory_instance.description == simple_food_data.get("description")
        assert factory_instance.photos == simple_food_data.get("photos")
        assert factory_instance.price == simple_food_data.get("price")
        assert factory_instance.slug == simple_food_data.get("slug")

    def test_if_reconstruct_instance_do_not_raise_validate_exception_with_incorrect_data(
        self, food_factory: IFoodFactory
    ):
        incorrect_data = {
            "id": -10,
            "name": None,
